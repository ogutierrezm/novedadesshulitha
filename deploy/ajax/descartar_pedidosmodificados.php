	...
	<div class="row">
							...
							<table id="datatable_tabletools" class="table table-striped table-bordered table-hover"  width="100%"  style="margin: 0px;padding:0;" border="0" cellspacing="0" cellpadding="0">
								<thead>
									<tr>
										...
										<th data-class="expand" class="noMostrarPrint" style=" font-family:Arial;font-size: 16px;text-align:center;">LIBERAR</th>
										<th data-class="expand" style=" font-family:Arial;font-size: 16px;text-align:center;"><strong><br/>PEDIDOS MODIFICADOS</strong></th>
									</tr>
								</thead>
								<tbody id="tBodyRpt">
									<?PHP
										$sRespuestaPorCobrar= generarReportePedidosModificadosAdmin();
		...
		<div class="col col-4 pull-right">	 
			<fieldset> 
				...
				<button id="btnRptEficiencia" class="btn btn-primary" type="button" onClick ="return MensajeSmartCorteDescartarModificados();">
...
<script type="text/javascript"> 
	...
	function noConsiderarModificados(){
		...
		for (var i = 0; i <= vConsecutivo ; i++) {
			...
			var vArray = ControlCheck.id.split('-');
			vFolioPedido = parseInt(vArray[1]);
			vSecuencia = parseInt(vArray[2]);
			if (ControlCheck.checked == true && vFolioPedido > 0 && vSecuencia > 0) {
				bRespuesta = DescartarModificado(vFolioPedido,vSecuencia);
				//bRespuesta = false;
			}else{
				continue;
			}
