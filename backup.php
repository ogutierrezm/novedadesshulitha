...
" " . $row[2] . ";";
  ...
  $res = pg_query("SELECT * FROM fn_trackpedidos1mes_delete() AS retorno;");
  while($row = pg_fetch_row($res))
  {
    $str .= "\n\n--\n";
    $str .= "-- Respuesta de eliminar folios tracks menores a la fecha hoy -30 días fn_trackpedidos1mes_delete ".$row[0];
    $str .= "\n--\n\n";
  }  
  fwrite($back,$str);
  fclose($back);
